...
private let log = Logger(subsystem: "uk.co.willdale.SynthiExtension", category: "AudioUnitViewController")
	...
	*/
	...
	nonisolated public func createAudioUnit(with componentDescription: AudioComponentDescription) throws -> AUAudioUnit {
		return try DispatchQueue.main.sync {
			...
			audioUnit = try SynthiExtensionAudioUnit(componentDescription: componentDescription, options: [])
			
			guard let audioUnit = self.audioUnit as? SynthiExtensionAudioUnit else {
				log.error("Unable to create SynthiExtensionAudioUnit")
				return audioUnit!
			}
			...
			audioUnit.setupParameterTree(SynthiExtensionParameterSpecs.createAUParameterTree())
    ...
    private func configureSwiftUIView(audioUnit: AUAudioUnit) {
        ...
        let content = SynthiExtensionMainView(parameterTree: observableParameterTree)
